...
def api_check_key():
    ...
    db = load_db()
    # Check server API key if one is set
    svk = db.get("___SERVER_API_KEY___", None)
    if svk:
        provided_svk = (request.values.get('server_key', '') or request.headers.get('X-Server-Key', '')).strip()
        if provided_svk != svk:
            return jsonify({"status": "unauthorized", "message": "Server key khong hop le hoac thieu server_key"})
...
def check_device_approval():
    ...
    svk = db.get("___SERVER_API_KEY___", None)
    if svk:
        provided_svk = (request.form.get('server_key', '') or request.headers.get('X-Server-Key', '')).strip()
        if provided_svk != svk:
            return jsonify({"status": "unauthorized", "msg": "Server key không hợp lệ hoặc thiếu server_key"})
    approved = db.get("___APPROVED_DEVICES___", {})
    if device_id not in approved:
...
# --- SERVER API KEY & URL MANAGEMENT ---

@app.route('/api/server_key', methods=['GET', 'POST'])
def manage_server_key():
    if not session.get('is_admin'): return jsonify({"status": "error"}), 401
    db = load_db()
    if request.method == 'GET':
        k = db.get("___SERVER_API_KEY___", None)
        return jsonify({"key": k})
    # POST → generate new key
    new_k = "SVK-" + _secrets_mod.token_hex(16).upper()
    db["___SERVER_API_KEY___"] = new_k
    save_db(db)
    return jsonify({"status": "success", "key": new_k})

@app.route('/api/server_key/delete', methods=['POST'])
def delete_server_key():
    if not session.get('is_admin'): return jsonify({"status": "error"}), 401
    db = load_db()
    if "___SERVER_API_KEY___" in db:
        del db["___SERVER_API_KEY___"]
        save_db(db)
    return jsonify({"status": "success"})

@app.route('/api/server_url', methods=['GET'])
def get_server_url():
    if not session.get('is_admin'): return jsonify({"status": "error"}), 401
    scheme = request.headers.get('X-Forwarded-Proto', request.scheme)
    host = request.headers.get('X-Forwarded-Host', request.host)
    base_url = f"{scheme}://{host}"
    return jsonify({"base_url": base_url, "status": "ok"})
...
<div class="nav-dropdown" id="navDrop">
  ...
  <button class="nav-item" onclick="sw('apiserver')"><i class="fa-solid fa-plug"></i> API Server</button>
  <div class="nav-divider"></div>
...
HTML_P3 = """
    ...
    <!-- TAB API SERVER -->
    <div id="tab-apiserver" class="tab">

      <!-- SECTION 1: URL SERVER -->
      <div class="card" style="background:linear-gradient(160deg,rgba(0,200,255,0.05),rgba(168,85,247,0.04));border-color:rgba(0,200,255,0.3);">
        <div class="card-title" style="border-bottom:1px solid rgba(0,200,255,0.2);padding-bottom:12px;margin-bottom:18px;">
          <i class="fa-solid fa-server" style="color:var(--blue);"></i> ĐỊA CHỈ KẾT NỐI API SERVER
        </div>

        <!-- Base URL -->
        <div style="margin-bottom:14px;">
          <div style="font-size:0.68rem;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:7px;"><i class="fa-solid fa-globe" style="color:var(--blue);margin-right:5px;"></i>URL Gốc Server</div>
          <div style="display:flex;gap:8px;align-items:center;">
            <input type="text" id="asvBaseUrl" readonly placeholder="Đang tải..." style="flex:1;padding:11px 14px;background:rgba(0,0,0,0.55);border:1px solid rgba(0,200,255,0.3);border-radius:9px;color:var(--success);font-weight:800;font-size:0.82rem;font-family:'Orbitron',sans-serif;outline:none;cursor:pointer;" onclick="asvCopy('asvBaseUrl',this)">
            <button class="btn-sm btn-sm-blue" onclick="asvCopy('asvBaseUrl',this)"><i class="fa-solid fa-copy"></i></button>
          </div>
        </div>

        <!-- Endpoint 1 -->
        <div style="margin-bottom:10px;">
          <div style="font-size:0.68rem;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:7px;">
            <span style="background:rgba(0,200,255,0.15);color:var(--blue);padding:2px 8px;border-radius:5px;margin-right:6px;">POST/GET</span>
            Endpoint Check Key
          </div>
          <div style="display:flex;gap:8px;align-items:center;">
            <input type="text" id="asvEp1" readonly placeholder="Đang tải..." style="flex:1;padding:11px 14px;background:rgba(0,0,0,0.55);border:1px solid rgba(0,200,255,0.2);border-radius:9px;color:#a8d8ff;font-size:0.78rem;font-family:monospace;outline:none;cursor:pointer;" onclick="asvCopy('asvEp1',this)">
            <button class="btn-sm btn-sm-blue" onclick="asvCopy('asvEp1',this)"><i class="fa-solid fa-copy"></i></button>
          </div>
          <div style="font-size:0.7rem;color:var(--muted);margin-top:5px;padding-left:4px;">Tham số: <code style="color:var(--success);background:rgba(0,0,0,0.3);padding:1px 5px;border-radius:3px;">key</code> + <code style="color:var(--success);background:rgba(0,0,0,0.3);padding:1px 5px;border-radius:3px;">device_id</code><span id="asvHint1" style="display:none;"> + <code style="color:var(--warn);background:rgba(0,0,0,0.3);padding:1px 5px;border-radius:3px;">server_key</code></span></div>
        </div>

        <!-- Endpoint 2 -->
        <div style="margin-bottom:14px;">
          <div style="font-size:0.68rem;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:7px;">
            <span style="background:rgba(168,85,247,0.15);color:var(--purple);padding:2px 8px;border-radius:5px;margin-right:6px;">POST</span>
            Endpoint Check Device ID
          </div>
          <div style="display:flex;gap:8px;align-items:center;">
            <input type="text" id="asvEp2" readonly placeholder="Đang tải..." style="flex:1;padding:11px 14px;background:rgba(0,0,0,0.55);border:1px solid rgba(168,85,247,0.2);border-radius:9px;color:#d0a8ff;font-size:0.78rem;font-family:monospace;outline:none;cursor:pointer;" onclick="asvCopy('asvEp2',this)">
            <button class="btn-sm btn-sm-blue" onclick="asvCopy('asvEp2',this)"><i class="fa-solid fa-copy"></i></button>
          </div>
          <div style="font-size:0.7rem;color:var(--muted);margin-top:5px;padding-left:4px;">Tham số: <code style="color:var(--success);background:rgba(0,0,0,0.3);padding:1px 5px;border-radius:3px;">device_id</code><span id="asvHint2" style="display:none;"> + <code style="color:var(--warn);background:rgba(0,0,0,0.3);padding:1px 5px;border-radius:3px;">server_key</code></span></div>
        </div>

        <!-- Response sample -->
        <div style="background:rgba(0,0,0,0.35);border:1px solid rgba(16,217,138,0.2);border-radius:10px;padding:12px 14px;">
          <div style="font-size:0.68rem;color:var(--success);font-weight:800;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px;"><i class="fa-solid fa-file-code" style="margin-right:5px;"></i>RESPONSE JSON MẪU</div>
          <div style="font-size:0.72rem;color:#88d4ab;font-family:monospace;line-height:2;">
            ✅ <span style="color:var(--success);">{"status":"success","message":"Key hop le","time_left":"7 ngày"}</span><br>
            ❌ <span style="color:var(--danger);">{"status":"invalid","message":"Key khong ton tai"}</span><br>
            ⏰ <span style="color:var(--warn);">{"status":"expired","message":"Key tren thiet bi nay da het han!"}</span><br>
            🔒 <span id="asvSvkSample" style="display:none;color:var(--warn);">{"status":"unauthorized","message":"Server key khong hop le..."}</span>
          </div>
        </div>
      </div>

      <!-- SECTION 2: SERVER API KEY -->
      <div class="card" style="margin-top:8px;background:linear-gradient(160deg,rgba(168,85,247,0.07),rgba(0,200,255,0.04));border-color:rgba(168,85,247,0.35);">
        <div class="card-title" style="border-bottom:1px solid rgba(168,85,247,0.22);padding-bottom:12px;margin-bottom:18px;">
          <i class="fa-solid fa-key" style="color:var(--purple);"></i> SERVER API KEY — BẢO VỆ ENDPOINT
        </div>

        <div style="background:rgba(168,85,247,0.06);border:1px solid rgba(168,85,247,0.18);border-radius:10px;padding:12px 14px;font-size:0.77rem;color:var(--muted);line-height:1.7;margin-bottom:16px;">
          <i class="fa-solid fa-circle-info" style="color:var(--purple);margin-right:5px;"></i>
          Khi tạo <strong style="color:var(--purple);">Server Key</strong>, mọi request tới các endpoint trên phải kèm tham số <code style="background:rgba(0,0,0,0.4);padding:2px 6px;border-radius:4px;color:var(--warn);">server_key=&lt;key&gt;</code> hoặc header <code style="background:rgba(0,0,0,0.4);padding:2px 6px;border-radius:4px;color:var(--warn);">X-Server-Key</code>. Chưa tạo = API mở tự do.
        </div>

        <!-- Key display -->
        <div style="margin-bottom:15px;">
          <div style="font-size:0.68rem;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px;">Server API Key hiện tại:</div>
          <div style="display:flex;gap:8px;align-items:center;">
            <input type="password" id="asvKeyInp" readonly placeholder="Chưa có — API đang mở (không cần xác thực)" style="flex:1;padding:12px 14px;background:rgba(0,0,0,0.6);border:1px solid rgba(168,85,247,0.35);border-radius:10px;color:var(--purple);font-weight:800;font-size:0.82rem;font-family:'Orbitron',sans-serif;outline:none;">
            <button class="btn-sm" id="asvEyeBtn" style="color:var(--muted);border-color:rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);min-width:38px;" onclick="asvToggleEye()"><i class="fa-solid fa-eye"></i></button>
            <button class="btn-sm btn-sm-blue" id="asvCopyKeyBtn" style="display:none;" onclick="asvCopyKey()"><i class="fa-solid fa-copy"></i></button>
          </div>
          <div id="asvKeyStatus" style="margin-top:8px;font-size:0.75rem;color:var(--warn);"><i class="fa-solid fa-lock-open"></i> API đang mở — không cần server_key</div>
        </div>

        <!-- Buttons -->
        <div style="display:flex;gap:9px;flex-wrap:wrap;margin-bottom:0;">
          <button class="btn btn-primary" style="flex:1;min-width:130px;margin-top:0;background:linear-gradient(135deg,#a855f7,#00c8ff);" onclick="asvGenKey()">
            <i class="fa-solid fa-wand-magic-sparkles"></i> TẠO / ĐỔI KEY MỚI
          </button>
          <button class="btn btn-danger" id="asvDelKeyBtn" style="flex:1;min-width:130px;margin-top:0;display:none;" onclick="asvDelKey()">
            <i class="fa-solid fa-unlock"></i> XÓA KEY (MỞ API)
          </button>
        </div>
        <div id="asvMsg" style="display:none;margin-top:12px;border-radius:10px;padding:10px 13px;font-size:0.82rem;font-weight:700;"></div>
      </div>
...
function sw(t) {
  ...
  if(t==='apiserver') loadApiServerTab();
}
...
// ---- API SERVER TAB FUNCTIONS ----
let _asvKeyVal = '';

function loadApiServerTab() {
  // Set URL fields immediately from window.location (always works)
  const base = window.location.origin;
  const el = document.getElementById('asvBaseUrl');
  if(!el) return;
  el.value = base;
  document.getElementById('asvEp1').value = base + '/api/check_key';
  document.getElementById('asvEp2').value = base + '/api/check_device_approval';
  // Also try server-side detection for behind-proxy URLs
  fetch('/api/server_url').then(r=>r.json()).then(d=>{
    if(d.base_url && d.base_url !== base) {
      document.getElementById('asvBaseUrl').value = d.base_url;
      document.getElementById('asvEp1').value = d.base_url + '/api/check_key';
      document.getElementById('asvEp2').value = d.base_url + '/api/check_device_approval';
    }
  }).catch(()=>{});
  asvLoadKey();
}
...
function asvLoadKey() {
  fetch('/api/server_key').then(r=>r.json()).then(d=>{
    const inp = document.getElementById('asvKeyInp');
    const copyBtn = document.getElementById('asvCopyKeyBtn');
    const delBtn = document.getElementById('asvDelKeyBtn');
    const statusEl = document.getElementById('asvKeyStatus');
    const h1 = document.getElementById('asvHint1');
    const h2 = document.getElementById('asvHint2');
    const svkSample = document.getElementById('asvSvkSample');
    if(d.key) {
      _asvKeyVal = d.key;
      inp.value = d.key;
      inp.placeholder = '';
      ...
      statusEl.innerHTML = '<i class="fa-solid fa-shield-check"></i> API đang được bảo vệ — cần kèm server_key';
      if(h1) h1.style.display = 'inline';
      if(h2) h2.style.display = 'inline';
      if(svkSample) svkSample.style.display = 'block';
    } else {
      _asvKeyVal = '';
      inp.value = '';
      inp.placeholder = 'Chưa có — API đang mở (không cần xác thực)';
      copyBtn.style.display = 'none';
      delBtn.style.display = 'none';
      statusEl.style.color = 'var(--warn)';
      statusEl.innerHTML = '<i class="fa-solid fa-lock-open"></i> API đang mở — không cần server_key';
      if(h1) h1.style.display = 'none';
      if(h2) h2.style.display = 'none';
      if(svkSample) svkSample.style.display = 'none';
    }
  }).catch(()=>{});
}

function asvGenKey() {
  fetch('/api/server_key', {method:'POST'}).then(r=>r.json()).then(d=>{
    if(d.status==='success') {
      asvShowMsg('purple', '<i class="fa-solid fa-key"></i> Đã tạo Server API Key mới! Hãy copy và lưu lại ngay.');
      asvLoadKey();
    }
  }).catch(()=>{ asvShowMsg('red','Lỗi kết nối server'); });
}

function asvDelKey() {
  if(!confirm('Xóa Server API Key? API sẽ mở hoàn toàn, không cần xác thực!')) return;
  fetch('/api/server_key/delete', {method:'POST'}).then(r=>r.json()).then(d=>{
    asvShowMsg('green', '<i class="fa-solid fa-lock-open"></i> Đã xóa Server Key. API hiện đang mở.');
    asvLoadKey();
  }).catch(()=>{ asvShowMsg('red','Lỗi kết nối server'); });
}

function asvCopyKey() {
  if(!_asvKeyVal) return;
  navigator.clipboard.writeText(_asvKeyVal).then(()=>{
    const btn = document.getElementById('asvCopyKeyBtn');
    const orig = btn.innerHTML;
    btn.innerHTML = '<i class="fa-solid fa-check"></i>';
    setTimeout(()=>{ btn.innerHTML = orig; }, 1400);
  }).catch(()=>{ prompt('Server API Key:', _asvKeyVal); });
}

function asvToggleEye() {
  const inp = document.getElementById('asvKeyInp');
  const btn = document.getElementById('asvEyeBtn');
  inp.type = inp.type==='password' ? 'text' : 'password';
  btn.innerHTML = inp.type==='password' ? '<i class="fa-solid fa-eye"></i>' : '<i class="fa-solid fa-eye-slash"></i>';
}

function asvCopy(id, btn) {
  const val = document.getElementById(id).value;
  if(!val || val==='Đang tải...') return;
  navigator.clipboard.writeText(val).then(()=>{
    const orig = btn.innerHTML;
    btn.innerHTML = '<i class="fa-solid fa-check"></i>';
    setTimeout(()=>{ btn.innerHTML = orig; }, 1200);
  }).catch(()=>{ prompt('Copy:', val); });
}

function asvShowMsg(color, html) {
  const el = document.getElementById('asvMsg');
  const colorMap = {purple:'rgba(168,85,247,0.12)', green:'rgba(16,217,138,0.1)', red:'rgba(239,68,68,0.1)'};
  const borderMap = {purple:'rgba(168,85,247,0.3)', green:'rgba(16,217,138,0.25)', red:'rgba(239,68,68,0.25)'};
  const textMap = {purple:'var(--purple)', green:'var(--success)', red:'var(--danger)'};
  el.style.cssText = `display:block;background:${colorMap[color]};border:1px solid ${borderMap[color]};color:${textMap[color]};border-radius:10px;padding:10px 13px;font-size:0.82rem;font-weight:700;`;
  el.innerHTML = html;
